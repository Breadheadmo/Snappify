...
const Slide = ({ slide, index, current, handleSlideClick, onButtonClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d] w-full h-full">
      <li
        ref={slideRef}
        className="flex flex-1 flex-col items-center justify-center relative text-center text-white opacity-100 transition-all duration-300 ease-in-out w-full h-full z-10"
...
const CarouselControl = ({ type, title, handleClick }: CarouselControlProps) => (
  <button
    ...
    onClick={handleClick}
  >
    <ArrowRight className="text-neutral-600 dark:text-neutral-200 h-5 w-5" />
  </button>
);
...
export function Carousel({ slides, onSlideButtonClick }: CarouselProps) {
  const [current, setCurrent] = useState(0);

  const handlePreviousClick = () => setCurrent((c) => (c - 1 < 0 ? slides.length - 1 : c - 1));
  const handleNextClick = () => setCurrent((c) => (c + 1 === slides.length ? 0 : c + 1));
  const handleSlideClick = (index: number) => current !== index && setCurrent(index);
  const id = useId();

  return (
    <div className="relative w-[70vmin] h-[70vmin] mx-auto overflow-hidden" aria-labelledby={`carousel-heading-${id}`}>
      <ul className="absolute inset-0 flex w-full h-full transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${current * 100}%)` }}>
        {slides.map((slide, index) => (
          <Slide key={index} slide={slide} index={index} current={current} handleSlideClick={handleSlideClick} onButtonClick={onSlideButtonClick} />
        ))}
      </ul>
