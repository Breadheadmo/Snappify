...
const sendOrderConfirmationEmail = async (order, user) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Order Confirmation - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #333; margin: 0;">Snappify</h1>
          ...
          <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
            <p style="color: #666; margin: 10px 0;">Thank you for shopping with Snappify!</p>
            ...
            <p style="color: #999; font-size: 12px; margin: 20px 0 0;">
              If you have any questions, contact us at ${process.env.SUPPORT_EMAIL || 'support@Snappify.co.za'}
            </p>
          </div>
        </div>
      </body>
    ...
    const mailOptions = {
      from: `"Snappify" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: `Order Confirmation #${order._id} - Snappify`,
      html: emailContent
    };
...
const sendOrderStatusEmail = async (order, user, oldStatus, newStatus) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Order Update - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
...
const sendPasswordResetEmail = async (user, resetToken) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Password Reset - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
          ...
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
            <h3 style="color: #333; margin-top: 0;">Hi ${user.firstName || user.username || 'there'},</h3>
    ...
    const mailOptions = {
      from: `"Snappify Support" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: 'Reset Your Password - Snappify',
      html: emailContent
    };
...
const sendLowStockAlert = async (products) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Low Stock Alert - Snappify Admin</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #333; margin: 0;">Snappify Admin</h1>
          ...
          <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
            <p style="color: #999; font-size: 12px; margin: 0;">
              This is an automated alert from your Snappify inventory management system.
            </p>
          </div>
        </div>
      </body>
    ...
    const mailOptions = {
      from: `"Snappify System" <${process.env.EMAIL_USER}>`,
      to: adminEmail,
...
const sendOrderTrackingEmail = async (order, user, trackingInfo) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Order Tracking Update - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
    ...
    const mailOptions = {
      from: `"Snappify Shipping" <${process.env.EMAIL_USER}>`,
      to: user.email,
...
const sendShippingConfirmationEmail = async (order, user, shippingDetails) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Your Order Has Shipped - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
...
const sendWelcomeEmail = async (user) => {
  try {
    ...
    const mailOptions = {
      from: `"Snappify Team" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: '🎉 Welcome to Snappify - Your Tech Shopping Journey Begins!',
...
const sendEmailVerification = async (user, verificationToken) => {
  try {
    ...
    const mailOptions = {
      from: `"Snappify Verification" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: '📧 Verify Your Email Address - Snappify',
...
const sendNewsletterConfirmation = async (email) => {
  try {
    ...
    const mailOptions = {
      from: `"Snappify Newsletter" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: '📰 Newsletter Subscription Confirmed - Snappify',
...
const sendOrderCancellationEmail = async (order, user, reason) => {
  try {
    ...
    const mailOptions = {
      from: `"Snappify Orders" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: `Order Cancelled - Order #${order._id} - Snappify`,
...
const sendWishlistReminder = async (user, wishlistItems) => {
  try {
    ...
    const mailOptions = {
      from: `"Snappify Wishlist" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: `💖 ${wishlistItems.length} Item${wishlistItems.length !== 1 ? 's' : ''} Waiting in Your Wishlist - Snappify`,
...
const sendSecurityAlert = async (user, alertType, details) => {
  try {
    ...
    const subjects = {
      login: '🔐 New Login Detected - Snappify',
      password_change: '✅ Password Changed Successfully - Snappify',
      email_change: '📧 Email Address Updated - Snappify',
      suspicious_activity: '⚠️ Security Alert - Snappify'
    };
    
    const mailOptions = {
      from: `"Snappify Security" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: subjects[alertType] || '🔒 Security Alert - Snappify',
...
const sendDeliveryNotification = async (order, user) => {
  try {
    ...
    const emailContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Package Delivered - Snappify</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 0; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #2563eb; margin: 0;">Snappify</h1>
          ...
          <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
            <p style="color: #666; margin: 10px 0;">Thank you for choosing Snappify!</p>
            <p style="color: #666; margin: 10px 0;">
    ...
    const mailOptions = {
      from: `"Snappify Delivery" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: `🎉 Package Delivered - Order #${order._id} - Snappify`,
      html: emailContent
    };
  ...
  try {
    ...
    for (let i = 0; i < recipients.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (recipient) => {
        try {
          const mailOptions = {
            from: `"Snappify Promotions" <${process.env.EMAIL_USER}>`,
            to: recipient.email,
            subject: subject,
...
const testEmailConfiguration = async () => {
  try {
    ...
    // Send test email
    const testEmail = {
      from: `"Snappify Test" <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER, // Send to self
      subject: '✅ Snappify Email Configuration Test',
      html: `
